...
Client::Client(FILE *config_file, FILE *config_priv, short port) : Node(config_file, config_priv, port)  {
  ...
  retransmit_request_timer = new ITimer(ctimeout*10000, retransmit_request_handler);
  start_commit_timer = new ITimer(atimeout*10000, start_commit_handler);
  //注释掉 retransmit_commit_timer = new ITimer(ctimeout, retransmit_commit_handler);
  retransmit_reply_timer = new ITimer(2*ctimeout*10000, retransmit_reply_handler);
